...
def read_image(image_path, is_uploaded=False):
    if is_uploaded:
        try:
            ...
            if img is None:
                logging.error(f"Failed to read uploaded image")
                return None
            return img
...
def save_image(image, filename, path="."):
  ...
  # Normalize path separators (handle Windows-style paths in config)
  norm_path = path.replace('\\', os.sep).replace('/', os.sep)
  # Ensure directory exists
  os.makedirs(norm_path, exist_ok=True)

  # Construct the full path
  full_path = os.path.join(norm_path, filename)
  ...
  if not success:
    logging.error(f"Failed to write image to {full_path}")
